...
export default function GameCard({ image, title }: GameCardProp) {
  ...
  return (
    <div className="group relative w-[121.6px] md:w-[152px] h-[160px] md:h-[200px] flex-shrink-0 rounded-[9.6px] md:rounded-[12px] overflow-hidden cursor-pointer">
      ...
      <button
        onClick={() => router.push(`/games/${title}`)}
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[48px] h-[48px] rounded-full bg-[#FFC83D] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-20"
      >
