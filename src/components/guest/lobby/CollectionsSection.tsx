    ...
    <section className="w-full flex flex-col gap-3 md:gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-[7.2px] md:gap-3">
          ...
          <h2 className="text-white font-jost font-extrabold uppercase tracking-[0.01em] text-[16px] leading-[23px] md:text-[20px] md:leading-[29px]">
            COLLECTIONS (17)
          </h2>
        </div>

        <button className="font-manrope text-[12px] leading-[16px] font-bold text-[#FFBF1F] md:text-[#D2DCF7]">
          View all
        </button>
      </div>

      {/* Cards */}
      <div className="flex gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory md:overflow-visible md:flex-nowrap">
        {collections.map((col) => (
          <div
            key={col.title}
          ...
          >
            ...
            {/* Title */}
            <span className="flex-1 text-center text-white font-jost font-extrabold tracking-[0.01em] text-[12px] leading-[17px] md:text-[22px] md:leading-[32px]">
              {col.title}
            </span>
          </div>
        ))}
